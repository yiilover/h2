...
$key = get_domain_key();
if($domain!="localhost")
if(is_bool(strpos( $key[1],md5( substr($domain,-intval($key[0])))))){die('Error:001');}
}
...
$client = new client(APP_PATH,$auth_key);
...
$string .='<select class="pc-select-'.$id.'" name="'.$id.'-'.$i.'" id="'.$id.'-'.$i.'" width="100"><option value="">请选择</option></select> ';
}
$string .= '<script type="text/javascript">
					$(function(){
...
function esf_hits($modelid,$id)
{
$hits_db = h5_base::load_model('hits_model');
$hits_r = $hits_db->get_one(array('hitsid'=>'c-'.$modelid.'-'.$id));
return $hits_r['dayviews'].'/'.$hits_r['yesterdayviews'].'/'.$hits_r['views'];
}
function get_hits($modelid,$id)
{
$hits_db = h5_base::load_model('hits_model');
$hits_r = $hits_db->get_one(array('hitsid'=>'c-'.$modelid.'-'.$id));
return $hits_r['views'];
...
function getcompany_info($id,$catid)
{
static $category;
if(empty($category)) {
$siteids = getcache('category_content','commons');
...
$tablename = $db->table_name;
$db->table_name = $tablename.'_data';
$rs = $db->get_one(array('id'=>$id),'content');
$r = array_merge($r,$rs);
return $r;
}
...
$house_db = h5_base::load_model('house_model');
$tablename = 'form_tuangou';
$db->change_table($tablename);
$r = $db->get_one(array('relation'=>$id),"COUNT(dataid) total");
$rs = $house_db->get_one(array('id'=>$id),"initialtuan");
if($rs)
{
$initialtuan = intval($rs['initialtuan']);
}
if(!$r ||empty($r['total'])) 
{
$r['total']= $initialtuan;
...
$tablename = 'form_tuangou';
$db->change_table($tablename);
$kanfang_db = h5_base::load_model('kanfang_model');
$r = $db->get_one(array('relatedid'=>$id,'type'=>1),"sum(num) total");
$rs = $kanfang_db->get_one(array('id'=>$id),"initialtuan");
if($rs)
{
$initialtuan = intval($rs['initialtuan']);
}
...
function get_piccount($id,$catid=0) {
$db = h5_base::load_model('content_model');
$siteid = get_siteid();
$categorys = getcache('category_content_'.$siteid,'commons');
$where = '1=1';
if(!$catid)
{
$huxing_num = get_piccount($id,13);
$photo_num = get_piccount($id,8);
return $huxing_num+$photo_num;
}
$category = $categorys[$catid];
$modelid = $category['modelid'];
$db->set_model($modelid);
...
$rs2 = $db->query("SELECT id FROM `$tablename` where $sql");
$result2 = $db->fetch_array($rs2);
if($result2)
{
...
$rs = $db->query("SELECT id,pictureurls FROM `$table_name` $sql2");
$key_array = $db->fetch_array($rs);
if(empty($key_array))
{
return 0;
}
...
function get_newscount($id,$catid=6) {
$db = h5_base::load_model('content_model');
$siteid = get_siteid();
$categorys = getcache('category_content_'.$siteid,'commons');
$where = '1=1';
...
function get_askcount($id,$catid=15) {
$db = h5_base::load_model('content_model');
$siteid = get_siteid();
$categorys = getcache('category_content_'.$siteid,'commons');
$where = "xglp=$id";
...
static $ftp;
$config = h5_base::load_config('remote');
$ftpon = $config['remote'];
if(!$ftpon) {
return $cmd == 'error'?-101 : 0;
}elseif($ftp == null) {
$ftp = &house5_ftp::instance();
}
if(!$ftp->enabled) {
return $ftp->error();
...
switch ($cmd) {
case 'upload': return $ftp->upload($config['attachdir'].'/'.$arg1,$arg1);break;
