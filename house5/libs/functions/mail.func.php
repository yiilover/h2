...
$headers = "From: $email_from{$maildelimiter}X-Priority: 3{$maildelimiter}X-Mailer: house5 {$maildelimiter}MIME-Version: 1.0{$maildelimiter}Content-type: text/html; charset=".CHARSET."{$maildelimiter}Content-Transfer-Encoding: base64{$maildelimiter}";
if(!$fp = stream_socket_client("tcp://".$mail['server'].":".$mail['port'],$errno,$errstr,30)) {
...
fputs($fp,($mail['auth'] ?'EHLO': 'HELO')." house5\r\n");
$lastmessage = fgets($fp,512);
