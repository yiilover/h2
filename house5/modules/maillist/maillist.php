...
$charset = h5_base::load_config('database','default');
$this->_charset = $charset['charset'];
h5_base::load_app_func ( 'global');
parent::__construct ();
$this->db = h5_base::load_model ( 'maillist_model');
...
showMessage(L('first_create_maillist'),'?s=maillist/maillist/maillist_create&menuid='.$menuid);
}
if (!empty ($this->maillist['code']) &&!empty ($this->maillist['sdid']) &&!empty ($this->maillist['group_addr'])) {
$menu = h5_base::load_model('menu_model');
...
$hash = md5(join('',$params) .'o.sdo');
$app_url = h5_base::load_config('system','app_path');
$pos_url = $app_url .'index.php?s=maillist/maillist/send_setting/menuid/'.$menuid .'/h5_hash/'.$h5_hash;
...
$hash = md5(join('',$params) .'o.sdo');
$app_url = h5_base::load_config('system','app_path');
$pos_url = $app_url .'index.php?s=maillist/maillist/user_mgr/menuid/'.$menuid .'/h5_hash/'.$h5_hash;
$en_url = base64_encode($pos_url);
...
$this->db->update($bind,"sdid='".$this->maillist['sdid'] ."'");
showMessage(L('update_succ'),'?s=maillist/maillist/maillist_mgr/menuid/'.$menuid);
}else {
showMessage(L('update_fail'),'?s=maillist/maillist/maillist_mgr/menuid/'.$menuid);
}
}
