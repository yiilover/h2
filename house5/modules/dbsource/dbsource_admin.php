...
$this->db = h5_base::load_model('dbsource_model');
parent::__construct();
...
$pages = $this->db->pages;
$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?s=dbsource/dbsource_admin/add\', title:\''.L('added_external_data_source').'\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',L('added_external_data_source'));
...
dbsource_cache();
showmessage(L('operation_success'),'?s=dbsource/dbsource_admin/init');
}else {
showmessage(L('operation_failure'),'?s=dbsource/dbsource_admin/init');
}
}else {
showmessage(L('notfound'),'?s=dbsource/dbsource_admin/init');
}
}
