...
h5_base::load_app_func('global','dianping');
class dianping extends admin {
function __construct() {
parent::__construct();
$this->dianping_type = h5_base::load_model('dianping_type_model');
$this->dianping_data = h5_base::load_model('dianping_data_model');
$this->dianping = h5_base::load_model('dianping_model');
$this->module = h5_base::load_model('module_model');
...
$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?s=dianping/dianping/add_type\', title:\'添加点评类型\', width:\'500\', height:\'350\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);','添加点评类型');
...
$default = $module ?$module : '不限模块';
$page = isset($_GET['page']) &&intval($_GET['page']) ?intval($_GET['page']) : 1;
$infos = $this->dianping_data->listinfo($where,$order = 'id DESC',$page,$pages = '8');
$pages = $this->dianping_data->pages;
$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?s=dianping/dianping/add_type\', title:\'添加点评类型\', width:\'500\', height:\'250\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);','添加点评类型');
...
$member_user_db = h5_base::load_app_class('member_interface','member');
...
showmessage(L('operation_success'),'?s=dianping/dianping');
}
}
}
...
setcache('dianping_type',$type_cache_array,'dianping');
showmessage(L('operation_success'),'?s=dianping/dianping/init','','edit');
}else{
$show_validator = $show_scroll = $show_header = true;
