...
$this->filepath = H5_PATH.'templates'.DIRECTORY_SEPARATOR.$this->style.DIRECTORY_SEPARATOR;
...
$this->tpl_edit = h5_base::load_config('system','tpl_edit');
parent::__construct();
}
...
$local = str_replace(array(H5_PATH,DIRECTORY_SEPARATOR.DIRECTORY_SEPARATOR),array('',DIRECTORY_SEPARATOR),$filepath);
if (substr($local,-1,1) == '.') {
...
$op_tag = h5_base::load_app_class($_GET['op']."_tag",$_GET['op']);
$html = $op_tag->$_GET['action']($_GET['html'],$_GET['value'],$_GET['id']);
echo $html;
}
public function edit_h5_tag() {
if (empty($this->tpl_edit)) {
showmessage(L('tpl_edit'));
}
...
$h5_tag = creat_h5_tag($op,array('url'=>$url,'cache'=>$cache,'return'=>$return));
if (in_array($tag_md5,$tag_md5_list[0])) {
$old_h5_tag = $tag_md5_list[1][$tag_md5];
}
if (replace_h5_tag($filepath,$old_h5_tag,$h5_tag,$this->style,$dir)) {
...
include $this->admin_tpl('h5_tag_tools_json_xml');
break;
case 'get':
if ($_POST['dosubmit']) {
...
$return = isset($_POST['return']) &&trim($_POST['return']) ?trim($_POST['return']) : '';
$tag_md5_list = tag_md5($filepath);
$h5_tag = creat_h5_tag($op,array('sql'=>$sql,'dbsource'=>$dbsource,'cache'=>$cache,'return'=>$return));
if (in_array($tag_md5,$tag_md5_list[0])) {
$old_h5_tag = $tag_md5_list[1][$tag_md5];
}
if (replace_h5_tag($filepath,$old_h5_tag,$h5_tag,$this->style,$dir)) {
...
$dbsource_db = h5_base::load_model('dbsource_model');
$r = $dbsource_db->select('','name');
$dbsource_list = array(''=>L('please_select'));
foreach ($r as $v) {
$dbsource_list[$v['name']] = $v['name'];
}
include $this->admin_tpl('h5_tag_tools_get');
break;
default:
if (!file_exists(H5_PATH.DIRECTORY_SEPARATOR.'modules'.DIRECTORY_SEPARATOR.$op.DIRECTORY_SEPARATOR.'classes'.DIRECTORY_SEPARATOR.$op.'_tag.class.php')) {
showmessage(L('the_module_will_not_support_the_operation'));
}
$op_tag = h5_base::load_app_class($op."_tag",$op);
if (!method_exists($op_tag,'h5_tag')) {
showmessage(L('the_module_will_not_support_the_operation'));
}
$html  = $op_tag->h5_tag();
if ($_POST['dosubmit']) {
...
include $this->admin_tpl('h5_tag_modules');
break;
}
}
}?>