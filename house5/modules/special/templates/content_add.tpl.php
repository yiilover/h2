...
<form name="myform" id="myform" action="?s=special/content/add&specialid=';echo $_GET['specialid'];echo '" method="post" enctype="multipart/form-data">
<div class="addContent">
<div class="crumbs">';echo L('pos_info');echo '</div>
<div class="col-right">
    	<div class="col-1">
...
};</script><input type="button" value="';echo L('cancel_thumb');echo '" onclick="$(\'#thumb_preview\').attr(\'src\',\'statics/images/icon/upload-pic.png\');$(\'#thumb\').val(\' \');return false;" class="button" style="width: 66px;"></div> 
	<h6> ';echo L('author');echo '</h6>
	 <input type="text" name="data[author]" value="" size="30"> 
	<h6> ';echo L('islink');echo '</h6>
	 <input type="text" name="linkurl" id="linkurl" value="" size="30" maxlength="255" disabled> <input name="info[islink]" type="checkbox" id="islink" value="1" onclick="ruselinkurl();" > <font color="red">';echo L('islink');echo '</font> 
	<h6> ';echo L('inputtime');echo '</h6> ';echo form::date('info[inputtime]',format::date(SYS_TIME,1) ,1);;echo '	<h6> ';echo L('template_style');echo '</h6> ';echo form::select($template_list,$style,'name="data[style]" id="style" onchange="load_file_list(this.value)"',L('please_select'));echo '	<h6> ';echo L('show_template');echo '</h6> <span id="show_template"><script type="text/javascript">$.getJSON(\'?s=admin/category/public_tpl_file_list&style=';echo $style;echo '&module=special&templates=show&id=';echo $show_template;echo '&name=data\', function(data){$(\'#show_template\').html(data.show_template);});</script></span>
          </div>
        </div>
    </div>
    <div class="col-auto">
...
<table width="100%" cellspacing="0" class="table_form">
	...
	<tr>
      ...
      <td>';echo form::select($types,'','name="info[typeid]" id="typeid"',L('please_choose_type'));echo '  </td>
    </tr>
	<tr>
    ...
    <tr>
      ...
      <td><input type=\'text\' name=\'info[keywords]\' id=\'keywords\' value=\'\' style=\'50\'  >  ';echo L('more_keywords_with_blanks');echo '</td>
    </tr>
	<tr>
      <th width="80"> ';echo L('description');echo '	  </th>
	...
	<tr>
      ...
      <td><select name="data[paginationtype]" id="paginationtype" onchange="if(this.value==1)$(\'#paginationtype1\').css(\'display\',\'\');else $(\'#paginationtype1\').css(\'display\',\'none\');">
                <option value="0">';echo L('no_page');echo '</option>
                <option value="1">';echo L('collate_copies');echo '</option>
                <option value="2">';echo L('manual_page');echo '</option>
            </select>
			<span id="paginationtype1" style="display:none"><input name="data[maxcharperpage]" type="text" id="maxcharperpage" value="10000" size="8" maxlength="8">';echo L('number_of_characters');echo '</span>  </td>
    </tr>
...
<div class="fixed-bottom">
    ...
    <div class="button"><input value="';echo L('save_and_add');echo '" type="submit" class="cu" name="dosubmit_continue" onclick="refersh_window();"></div>
...
$(function(){
	...
	$("#typeid").formValidator({autotip:true,onshow:"';echo L('please_choose_type');echo '",onfocus:"';echo L('please_choose_type');echo '"}).inputValidator({min:1,onerror:"';echo L('please_choose_type');echo '"});
	$("#title").formValidator({autotip:true,onshow:"';echo L('please_input_title');echo '",onfocus:"';echo L('please_input_title');echo '"}).inputValidator({min:1,onerror:"';echo L('please_input_title');echo '"});
