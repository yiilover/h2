...
$url = $site_info['domain'].'index.php?s=special/index&id='.$id;
}
}else {
$url = $special['ishtml'] ?APP_PATH.substr(h5_base::load_config('system','html_root'),1).'/special/'.$special['filename'].'/': APP_PATH.'index.php?s=special/index&id='.$id;
}
$this->db->update(array('url'=>$url,'listorder'=>$id),array('id'=>$id,'siteid'=>$this->get_siteid()));
if ($special['ishtml']) {
...
$special['url'] = APP_PATH.substr(h5_base::load_config('system','html_root'),1).'/special/'.$special['filename'].'/';
}
}elseif ($special['ishtml']=='0') {
if ($siteid>1) {
$special['url'] = $site_info['domain'].'index.php?s=special/index&specialid='.$_GET['specialid'];
}else {
$special['url'] = APP_PATH.'index.php?s=special/index&specialid='.$_GET['specialid'];
}
}
...
showmessage(L('update_special_success'),'?s=special/special/init');
}
}
private function create_index($specialid) {
$info = $this->db->get_one(array('id'=>$specialid));
if (!$info['ishtml']) {
showmessage($info['title'].L('update_success'),'?s=special/special/public_create_html');
}
$html = h5_base::load_app_class('html');
$html->_index($specialid);
showmessage($info['title'].L('index_update_success'),'?s=special/special/public_create_type&specialid='.$specialid);
}
...
if (!$pages) {
$c = h5_base::load_model('special_content_model');
...
showmessage($typename.L('type_update_success'),'?s=special/special/public_create_content&specialid='.$specialid);
}
if ($pages<=$maxpage) {
showmessage($typename.L('update_success'),'?s=special/special/public_create_type&specialid='.$specialid);
}else {
showmessage($typename.L('type_from').($_GET['page'] ?$_GET['page'] : 1).L('type_end').$maxpage.'</font> '.L('update_success'),'?s=special/special/public_create_type&typeid='.$typeid.'&typename='.$typename.'&page='.$page.'&pages='.$pages.'&specialid='.$specialid);
}
}else {
$special_api = h5_base::load_app_class('special_api');
$types = $special_api->_get_types($specialid);
setcache('create_types',$types,'commons');
showmessage(L('start_update_type'),'?s=special/special/public_create_type&specialid='.$specialid);
}
}
...
if ($page>=$pages) {
showmessage(L('content_update_success'),'?s=special/special/public_create_html&specialid='.$specialid);
}else {
$page++;
showmessage(L('content_from').' <font color="red">'.intval($offset+1).L('type_end').intval($offset+10).'</font> '.L('update_success'),'?s=special/special/public_create_content&specialid='.$specialid.'&page='.$page.'&pages='.$pages);
}
}
...
$vote_db = h5_base::load_model('vote_subject_model');
