...
?>
<div class="pad-10">
<form method="post" action="?s=announce/admin_announce/edit/aid/<?php echo $_GET['aid']?>" name="myform" id="myform">
...
<tbody>
	...
	<tr>
		...
		<td  id="show_template"><?php if ($an_info['style']) echo '<script type="text/javascript">$.getJSON(\'?s=admin/category/public_tpl_file_list/style/'.$an_info['style'].'/id/'.$an_info['show_template'].'/module/announce/templates/show/name/announce/h5_hash/\'+h5_hash, function(data){$(\'#show_template\').html(data.show_template);});</script>'?></td>
	</tr>
	<tr>
...
function load_file_list(id) {
	$.getJSON('?s=admin/category/public_tpl_file_list/style/'+id+'/module/announce/templates/show/name/announce/h5_hash/'+h5_hash, function(data){$('#show_template').html(data.show_template);});
}

$(document).ready(function(){
	$.formValidator.initConfig({formid:"myform",autotip:true,onerror:function(msg,obj){window.top.art.dialog({content:msg,lock:true,width:'220',height:'70'}, function(){this.close();$(obj).focus();})}});
	$('#title').formValidator({onshow:"<?php echo L('input_announce_title')?>",onfocus:"<?php echo L('title_min_3_chars')?>",oncorrect:"<?php echo L('right')?>"}).inputValidator({min:1,onerror:"<?php echo L('title_cannot_empty')?>"}).ajaxValidator({type:"get",url:"",data:"s=announce/admin_announce/public_check_title/aid/<?php echo $_GET['aid']?>",datatype:"html",cached:false,async:'true',success : function(data) {
        if( data == "1" )
		{
            return true;
		}
