...
$this->db = h5_base::load_model('link_model');
$this->db2 = h5_base::load_model('type_model');
}
...
$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?s=link/link/add\', title:\''.L('link_add').'\', width:\'700\', height:\'450\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',L('link_add'));
include $this->admin_tpl('link_list');
}
public function public_name() {
$link_title = isset($_GET['link_name']) &&trim($_GET['link_name']) ?(h5_base::load_config('system','charset') == 'gbk'?iconv('utf-8','gbk',trim($_GET['link_name'])) : trim($_GET['link_name'])) : exit('0');
...
$type_name = isset($_GET['type_name']) &&trim($_GET['type_name']) ?(h5_base::load_config('system','charset') == 'gbk'?iconv('utf-8','gbk',trim($_GET['type_name'])) : trim($_GET['type_name'])) : exit('0');
...
if(h5_base::load_config('system','attachment_stat') &$_POST['link']['logo']) {
...
showmessage(L('operation_success'),'?s=link/link/edit','','edit');
}else{
$show_validator = $show_scroll = $show_header = true;
h5_base::load_sys_class('form','',0);
...
showmessage(L('operation_success'),'?s=link/link/list_type','','edit');
}else{
$show_validator = $show_scroll = $show_header = true;
...
showmessage(L('operation_success'),'?s=link/link/init');
}else{
$linkid = intval($_GET['linkid']);
if($linkid <1) return false;
...
showmessage(L("operation_failure"),'?s=link/link/init');
}
}
showmessage(L('operation_success'),HTTP_REFERER);
}
...
showmessage(L('setting_updates_successful'),'?s=link/link/init');
}else {
@extract($now_seting);
$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?s=link/link/add\', title:\''.L('link_add').'\', width:\'700\', height:\'450\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',L('link_add'));
