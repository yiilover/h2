...
$this->db = h5_base::load_model('badword_model');
parent::__construct();
}
function init () {
...
$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?s=admin/badword/add\', title:\''.L('badword_add').'\', width:\'450\', height:\'180\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',L('badword_add'));
...
showmessage(L('enter_word'),'?s=admin/badword/add');
}
$this->db->insert($_POST['info']);
$this->public_cache_file();
showmessage(L('operation_success'),'?s=admin/badword/add','','add');
}else{
...
$badword = isset($_GET['badword']) &&trim($_GET['badword']) ?(h5_base::load_config('system','charset') == 'gbk'?iconv('utf-8','gbk',trim($_GET['badword'])) : trim($_GET['badword'])) : exit('0');
...
showmessage(L('operation_success'),'?s=admin/badword/init');
}
function edit() {
if(isset($_POST['dosubmit'])){
...
$this->public_cache_file();
showmessage(L('operation_success'),'?s=admin/badword/edit','','edit');
}else{
$show_validator = $show_scroll = $show_header = true;
$info = array();
...
showmessage(L("operation_failure"),'?s=admin/badword/init');
}
}
}
function export() {
...
showmessage('暂无敏感词设置，正在返回！','?s=admin/badword/init');
}
foreach($result as $s){
extract($s);
...
$arr = $s = $str = $level_arr = '';
$s = trim($_POST['info']);
if(empty($s)) showmessage(L('not_information'),'?s=admin/badword/import');
$arr = explode("\n",$s);
