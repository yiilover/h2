...
<!--
	$(function(){
		...
		$("#name").formValidator({onshow:"';echo L('input').L('release_point_name');echo '",onfocus:"';echo L('input').L('release_point_name');echo '"}).inputValidator({min:1,onerror:"';echo L('input').L('release_point_name');echo '"}).ajaxValidator({type : "get",url : "",data :"s=admin/release_point/public_name",datatype : "html",async:\'false\',success : function(data){	if( data == "1" ){return true;}else{return false;}},buttons: $("#dosubmit"),onerror : "';echo L('release_point_name').L('exists');echo '",onwait : "';echo L('connecting');echo '"});
		...
		$("#port").formValidator({onshow:"';echo L('input').L('server_port');echo '",onfocus:"';echo L('input').L('server_port');echo '",defaultvalue:\'21\'}).inputValidator({min:1,onerror:"';echo L('input').L('server_port');echo '"}).regexValidator({datatype:\'enum\',regexp:\'intege1\',onerror:\'';echo L('server_ports_must_be_integers');echo '\'}).defaultPassed();
...
<div class="pad-10">
<form action="?s=admin/release_point/add" method="post" id="myform">
<fieldset>
...
<fieldset>
  ...
  <tr>
   ...
   <tr>
    ...
    <td class="y-bg"><label><input type="checkbox" class="inputcheckbox" name="pasv" value="1" id="pasv" size="30" />';echo L('yes');echo '</label></td>
  </tr>
    <tr>
    <th>';echo L('ssl_connection');echo '£º</th>
    <td class="y-bg"><label><input type="checkbox" class="inputcheckbox" name="ssl" value="1" id="ssl" size="30" ';if(!$this->ssl){echo 'disabled';};echo ' />';echo L('yes');echo '</label> ';if(!$this->ssl){echo '<span style="color:red">'.L('your_server_will_not_support_the_ssl_connection').'</a>';};echo '</td>
  </tr>
    </tr>
    <tr>
