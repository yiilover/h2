...
if (file_exists(H5_PATH.'model'.DIRECTORY_SEPARATOR.$model.'_model.class.php')) {
$this->db = h5_base::load_model($model.'_model');
if ($param) {
$this->$model($param);
}else {
$this->$model();
...
$priv_db = h5_base::load_model('admin_role_priv_model');
$sitelist = array();
foreach($role as $n=>$r) {
...
define('MODEL_PATH',H5_PATH.'modules'.DIRECTORY_SEPARATOR.'content'.DIRECTORY_SEPARATOR.'fields'.DIRECTORY_SEPARATOR);
...
define('MEMBER_MODEL_PATH',H5_PATH.'modules'.DIRECTORY_SEPARATOR.'member'.DIRECTORY_SEPARATOR.'fields'.DIRECTORY_SEPARATOR);
define('MEMBER_CACHE_MODEL_PATH',HOUSE5_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_model'.DIRECTORY_SEPARATOR.'caches_data'.DIRECTORY_SEPARATOR);
...
if (is_array($M)) {
foreach ($M as $key =>$m) {
if (file_exists(H5_PATH.'modules'.DIRECTORY_SEPARATOR.$key.DIRECTORY_SEPARATOR.'classes'.DIRECTORY_SEPARATOR.$key.'_tag.class.php') &&!in_array($key,array('message','block'))) {
$module[$key] = $m['name'];
}
}
}
...
return $file_size = h5_base::load_config('system','lock_ex') ?file_put_contents($filepath.'modules.php',$module,LOCK_EX) : file_put_contents($filepath.'modules.php',$module);
}
public function cache2database() {
$cache = h5_base::load_model('cache_model');
...
$filesize = h5_base::load_config('system','lock_ex') ?file_put_contents(CACHE_PATH.$re['path'].$re['filename'],$re['data'],LOCK_EX) : file_put_contents(CACHE_PATH.$re['path'].$re['filename'],$re['data']);
}else {
continue;
}
}
...
showmessage(L('part_cache_success'),'?s=admin/cache_all/init/page/'.$_GET['page'].'/currpage/'.$currpage.'/pages/'.$pages.'/dosubmit/1',0);
}
}
public function del_file() {
$path = HOUSE5_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_template'.DIRECTORY_SEPARATOR;
$files = glob($path.'*');
...
$path = HOUSE5_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_tpl_data'.DIRECTORY_SEPARATOR.'caches_data'.DIRECTORY_SEPARATOR;
$files = glob($path.'*');
if (is_array($files)) {
foreach ($files as $f) {
$dir = basename($f);
