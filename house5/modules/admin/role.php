...
$this->op = h5_base::load_app_class('role_op');
}
...
$this->_cache();
if($insert_id){
showmessage(L('operation_success'),'?s=admin/role/init');
}
}else {
...
$roleid = intval($_GET['roleid']);
if ($siteid) {
$menu = h5_base::load_sys_class('tree');
$menu->icon = array('©¦ ','©À©¤ ','©¸©¤ ');
$menu->nbsp = '&nbsp;&nbsp;&nbsp;';
$result = $this->menu_db->select();
$priv_data = $this->priv_db->select();
...
h5_base::load_app_class('role_cat','',0);
...
showmessage(L('operation_success'),'?s=admin/role/init','','edit');
break;
default:
$sites = h5_base::load_app_class('sites','admin');
$sites_list = $sites->get_list();
