...
$this->cache_api = h5_base::load_app_class('cache_api','admin');
$m = $modules[$page];
if ($m['mod'] &&$m['function']) {
if ($m['file'] == '') $m['file'] = $m['function'];
$M = getcache('modules','commons');
if (in_array($m['mod'],array_keys($M))) {
$cache = h5_base::load_app_class($m['file'],$m['mod']);
$cache->$m['function']();
}
...
showmessage(L('update').$m['name'].L('cache_file_success'),'?s=admin/cache_all//page/'.$page.'/dosubmit/1/h5_hash/'.$_SESSION['h5_hash'],0);
}else {
