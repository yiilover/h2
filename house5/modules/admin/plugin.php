...
h5_base::load_sys_func('plugin');
class plugin extends admin {
private $db,$db_var;
function __construct() {
parent::__construct();
$this->db = h5_base::load_model('plugin_model');
...
if(file_exists(H5_PATH.'plugin'.DIRECTORY_SEPARATOR.$_r['dir'].DIRECTORY_SEPARATOR.$_r['dir'].'.class.php')){
...
$pluginsdir = dir(H5_PATH.'plugin');
while (false !== ($entry = $pluginsdir->read())) {
$config_file = '';
$plugin_data = array();
if(!in_array($entry,array('.','..')) &&is_dir(H5_PATH.'plugin'.DIRECTORY_SEPARATOR.$entry) &&!in_array($entry,$installsdir) &&!$this->db->get_one(array('identification'=>$entry))) {
$config_file = H5_PATH.'plugin'.DIRECTORY_SEPARATOR.$entry.DIRECTORY_SEPARATOR.'plugin_'.$entry.'.cfg.php';
...
$dir = trim($_GET['dir']);
$license = 0;
$config_file = H5_PATH.'plugin'.DIRECTORY_SEPARATOR.$dir.DIRECTORY_SEPARATOR.'plugin_'.$dir.'.cfg.php';
if(file_exists($config_file)) {
$plugin_data = @require($config_file);
$license = ($plugin_data['license'] == ''||!isset($plugin_data['license'])) ?0 : 1;
}
if(empty($_GET['license']) &&$license) {
$submit_url = '?s=admin/plugin/import/dir/'.$dir.'/license/1/h5_hash/'.$_SESSION['h5_hash'].'/menuid/'.$_GET['menuid'];
}else {
$submit_url = '?s=admin/plugin/install/dir/'.$dir.'/h5_hash/'.$_SESSION['h5_hash'].'/menuid/'.$_GET['menuid'];
}
$show_header = 0;
...
$filename = H5_PATH.'plugin'.DIRECTORY_SEPARATOR.$dir.DIRECTORY_SEPARATOR.$plugin_data['plugin']['uninstallfile'];
...
if(!dir_delete(H5_PATH.'plugin'.DIRECTORY_SEPARATOR.$dir)) {
showmessage(L('plugin_uninstall_success_no_delete','','plugin'),'?s=admin/plugin/init');
}
}
showmessage(L('plugin_uninstall_success','','plugin'),'?s=admin/plugin/init');
}else {
showmessage(L('plugin_uninstall_fail','','plugin'),'?s=admin/plugin/init');
}
}else {
$show_header = 0;
...
if($plugin_data['version'] &&$plugin_data['version']!=h5_base::load_config('version','h5_version')) {
...
$filename = H5_PATH.'plugin'.DIRECTORY_SEPARATOR.$dir.DIRECTORY_SEPARATOR.$plugin_data['plugin']['installfile'];
if(file_exists($filename)) {
@include_once $filename;
}
if($op_status) {
...
showmessage(L('plugin_install_success','','plugin'),'?s=admin/plugin/init');
}else {
showmessage(L('plugin_install_fail','','plugin'),'?s=admin/plugin/init');
}
}
...
$this->set_hook_cache();
showmessage(L('operation_success'),'?s=admin/plugin/init');
}else {
showmessage(L('operation_failure'),'?s=admin/plugin/init');
}
}
...
$filepath = H5_PATH.'plugin'.DIRECTORY_SEPARATOR.$data['identification'].DIRECTORY_SEPARATOR.'plugin_'.$data['identification'].'.cfg.php';
h5_base::load_sys_func('dir');
...
$s = 8;
$data = file_get_contents('http://open.phpcms.cn/index.php?s=open/api/get_applist/s/'.$s.'/p/'.$p);
$data = array_iconv(json_decode($data,true),'utf-8',CHARSET);
$recommed_data = file_get_contents('http://open.phpcms.cn/index.php?s=open/api/get_recommed_applist/s/5/p/1');
$recommed_data = array_iconv(json_decode($recommed_data,true),'utf-8',CHARSET);
$focus_data = file_get_contents('http://open.phpcms.cn/index.php?s=open/api/get_app_focus/num/3');
$focus_data = array_iconv(json_decode($focus_data,true),'utf-8',CHARSET);
$pages = $data['pages'];
$pre_page = $p <= 1 ?1 : $p -1;
$next_page = $p >= $pages ?$pages : $p +1;
$pages = '<a class="a1">'.$data['total'].L('plugin_item','','plugin').'</a> <a href="?s=admin/plugin/appcenter/p/1">'.L('plugin_firstpage').'</a> <a href="?s=admin/plugin/appcenter/p/'.$pre_page.'">'.L('plugin_prepage').'</a> <a href="?s=admin/plugin/appcenter/p/'.$next_page.'">'.L('plugin_nextpage').'</a> <a href="?s=admin/plugin/appcenter/p/'.$pages.'">'.L('plugin_lastpage').'</a>';
$show_header = 1;
...
$data = array();
$id = intval($_GET['id']);
$data = file_get_contents('http://open.phpcms.cn/index.php?s=open/api/get_detail_byappid/id/'.$id);
$data = array_iconv(json_decode($data,true),'utf-8',CHARSET);
extract($data);
if($appname) {
...
$appdirname = H5_PATH.'plugin'.DIRECTORY_SEPARATOR.$data['appenname'];
...
showmessage(L('plugin_configure_success','','plugin'),'index.php?s=admin/plugin/import/dir/'.$data['appenname']);
}else {
...
showmessage(L('download_fail','','plugin'),'index.php?s=admin/plugin/appcenter');
}
...
$copy_to_pcpath = H5_PATH.'plugin';
$copy_to_staticspath = HOUSE5_PATH.'statics'.DIRECTORY_SEPARATOR.'plugin';
$appdirname = $data['appenname'];
$this->copyfailnum = 0;
$this->copydir($copy_from.DIRECTORY_SEPARATOR.'house5'.DIRECTORY_SEPARATOR.'plugin',$copy_to_pcpath,$_GET['cover']);
...
showmessage(L('download_success','','plugin'),'index.php?s=admin/plugin/import/dir/'.$appdirname);
}
}
}
...
$plugin_admin_path = H5_PATH.'plugin'.DIRECTORY_SEPARATOR.$identification.DIRECTORY_SEPARATOR.'plugin_admin.class.php';
...
foreach($info as $i) {
$id = $i['identification'];
$hook_file = H5_PATH.'plugin'.DIRECTORY_SEPARATOR.$id.DIRECTORY_SEPARATOR.'hook.class.php';
