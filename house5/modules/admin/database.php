...
$database = h5_base::load_config('database');
if($_GET['dosubmit']) {
$admin_founders = explode(',',h5_base::load_config('system','admin_founders'));
...
if(preg_match("/(house5tables_[0-9]{8}_[0-9a-z]{4}_)([0-9]+)\.sql/i",basename($sqlfile),$num)) {
...
showmessage(L('operation_success'),'?s=admin/database/export/pdoname/'.$pdo_name);
...
showmessage(L('select_tbl'),'?s=admin/database/export/pdoname/'.$pdo_name);
}
}
...
showmessage(L('operation_success'),'?s=admin/database/import/pdoname/'.$pdo_name);
}else {
...
return array('house5tables'=>$house5,'othertables'=>$other);
}
...
if(trim($tabledump)) {
$tabledump = "# house5 bakfile\n# version:house5\n# time:".date('Y-m-d H:i:s')."\n# type:house5\n# house5:http://www.house5.net\n# --------------------------------------------------------\n\n\n".$tabledump;
$tableid = $i;
...
showmessage(L('bakup_file')." $filename ".L('bakup_write_succ'),'?s=admin/database/export/sizelimit/'.$sizelimit.'/sqlcompat/'.$sqlcompat.'/sqlcharset/'.$sqlcharset.'/tableid/'.$tableid.'/fileid/'.$fileid.'/startfrom/'.$startrow.'/random/'.$random.'/dosubmit/1/tabletype/'.$tabletype.'/allow/'.$allow.'/pdo_select/'.$this->pdo_name);
}else {
...
delcache('bakup_tables','commons');
showmessage(L('bakup_succ'),'?s=admin/database/import/pdoname/'.$this->pdo_name);
}
}
...
$fileid++;
showmessage(L('bakup_data_file')." $filename ".L('load_success'),"?s=admin/database/import/pdoname/".$this->pdo_name."/pre/".$pre."/fileid/".$fileid."/dosubmit/1");
}else {
showmessage(L('data_recover_succ'),'?s=admin/database/import');
}
}
}
...
if($this->db_tablepre != "h5_") $sql = str_replace("`h5_",'`'.$this->db_tablepre,$sql);
$sql = str_replace("\r","\n",$sql);
$ret = array();
$num = 0;
