...
if($minlength &&$advantage_length <$minlength) showmessage("优点不得少于 $minlength 个字符！");
...
if($minlength &&$disadvantage_length <$minlength) showmessage("缺点不得少于 $minlength 个字符！");
if($maxlength &&$disadvantage_length >$maxlength) {
showmessage("缺点不得超过 $maxlength 个字符！");
}
$length = empty($content) ?0 : strlen($content);
if($minlength &&$length <$minlength) showmessage("总结不得少于 $minlength 个字符！");
