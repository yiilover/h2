...
switch ($searchtype) {
case '0':
$sql = "SELECT `commentid` FROM `house5_comment` WHERE `siteid` = '$this->siteid' AND `title` LIKE '%$keywords%' AND `tableid` = '$tableid' ";
...
$sql = "SELECT `commentid` FROM `house5_comment` WHERE `commentid` LIKE 'content_%-$keywords-%' ";
$this->comment_db->query($sql);
$data = $this->comment_db->fetch_array();
if (!empty($data)) {
foreach ($data as $d) {
