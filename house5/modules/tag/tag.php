...
$this->db = h5_base::load_model('tag_model');
$this->dbsource = h5_base::load_model('dbsource_model');
parent::__construct();
}
public function init() {
$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?s=tag/tag/add\', title:\''.L('add_tag').'\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',L('add_tag'));
...
$html = h5_tag_class($module);
$data = array();
$tag = '{h5:'.$module.' action="'.$action.'" ';
...
$modules = array_merge(array(''=>L('please_select')),h5_base::load_config('modules'));
...
if ($module) $html = h5_tag_class($module);
