...
$catid_str = $this->category[$v]['arrchildid'];
$pos = strpos($catid_str,',')+1;
if(empty($catids_str))
{
$catids_str= substr($catid_str,$pos);
}
else
{
$catids_str= $catids_str.','.substr($catid_str,$pos);
}
}
else
{
...
$key_array = $this->db->select($sql,'id,title,catid,description,url,inputtime',$limit,$order,'','id');
}
unset($result);
}
return $key_array;
...
$ids = implode(',',$ids);
$sql.= " and `id` in ($ids)";
if(isset($data['type'])) $sql .=  " and `type`=$data[type]";
$this->db->table_name = $tablename;
$key_array = $this->db->select($sql,'id,price,addtime,trend,type',$limit,$order,'','id');
}
}
return $key_array;
}
...
$this->db->table_name = $tablename;
$key_array = $this->db->select($sql,'id,typeid,title,inputtime',$limit,$order,'','id');
}
if($key_array)
{
...
$this->db->table_name = $this->tablename;
$result = $this->db->select($sql,'id,title,hot,character,region,thumb,tel,price,priceunit,url,opentime,opentimetips,address,dzinfo',$data['limit'],'','','id');
}
return $result;
}
...
{
$this->__db = h5_base::load_model('sitemodel_model');
$siteid = $data['siteid'] &&intval($data['siteid']) ?intval($data['siteid']) : get_siteid();
$formid = intval($data['formid']);
$num = intval($data['limit'])?intval($data['limit']):10;
$r = $this->__db->get_one(array('modelid'=>$formid,'siteid'=>$siteid,'disabled'=>0),'tablename, setting');
$this->tablename = 'form_'.$r['tablename'];
$rs = $this->__db->query("select count(relation) as counts,relation,subject,regionid from h5_form_tuangou where `type`=0 group by relation order by count(relation) desc limit ".$num);
$result = $this->__db->fetch_array($rs);
return $result;
}
public function toptuangou($data) 
{
$this->__db = h5_base::load_model('sitemodel_model');
$siteid = $data['siteid'] &&intval($data['siteid']) ?intval($data['siteid']) : get_siteid();
$formid = '17';
...
$rs = $this->__db->query("select count(relation) as counts,relation,subject,regionid from h5_form_tuangou where `type`=0 $where group by relation order by count(relation) desc limit ".$num);
$result = $this->__db->fetch_array($rs);
return $result;
}
...
$r = $this->__db->get_one(array('modelid'=>$formid,'siteid'=>$siteid,'disabled'=>0),'tablename, setting');
$this->tablename = 'form_'.$r['tablename'];
$rs = $this->__db->query("select datetime,tel,subject,name,qq,description from h5_form_tuangou $where order by datetime desc limit ".$num);
$result = $this->__db->fetch_array($rs);
return $result;
}
...
public function huxingthumb($data){
$catid = intval($data['catid']);
if(!$this->set_modelid($catid)) return false;
$tablename = $this->db->table_name;
$table_name = $tablename.'_data';
...
public function community_houseinfo($data) {
$catid = intval($data['catid']);
if(!$this->set_modelid($catid)) return false;
$tablename = $this->db->table_name;
$table_name = $tablename.'_data';
...
public function esflists($data) {
$catid = intval($data['catid']);
if(!$this->set_modelid($catid)) return false;
$tablename = $this->db->table_name;
$sql = "`status`=99 and house_status=1";
$ids = array();
$page = intval($data['pg'])?intval($data['pg']):1;
$limit = intval($data['limit'])?intval($data['limit']):9;
if($catid=='112')
{
$urlrules = ESF_PATH.'mendian-esf-'.$data['userid'].'-g{$page}';
}
elseif($catid=='113')
{
$urlrules = ESF_PATH.'mendian-rent-'.$data['userid'].'-g{$page}';
}
define('URLRULE',$urlrules);
if($data['where'])
{
$this->_db = h5_base::load_model('member_model');
$this->_db->set_model(42);
$where = '1=1 '.$data['where'];
$order = "userid desc";
$this->_member_modelinfo = $this->_db->select($where,"*",$limit,$order);
foreach($this->_member_modelinfo as $key =>$memberinfo)
{
$ids[]= $memberinfo['userid'];
}
$ids = implode(',',$ids);
if($ids)
{
...
if($data['other'])
{
$sql = $data['other']." and `uid` in ($ids)";
}
if($data['recommend'])
{
$sql.= " and recommend=1";
}
$return = $this->db->listinfo($sql,$order,$page,$limit);
if($data['other'])
{
$pages = $this->db->pages;
}
...
{
$ids[]= $r['id'];
$groupid = get_nickname($r['uid'],'groupid');
$return[$key]['groupname'] = $grouplist[$groupid]['name'];
$return[$key]['groupicon'] = $grouplist[$groupid]['icon'];
$return[$key]['point'] = get_nickname($r['uid'],'point');
}
$ids = implode(',',$ids);
if (!empty($ids)) {
$this->db->table_name = $this->db->table_name.'_data';
...
public function communitylists($data) {
$catid = intval($data['catid']);
if(!$this->set_modelid($catid)) return false;
$tablename = $this->db->table_name;
$table_name = $tablename.'_data';
...
$where = "and e.uid=$uid";
if(!$this->set_modelid($catid)) return false;
$rs = $this->db->query("select e.communityname,d.relation,count(d.relation) as counts from ".$tablename." e inner join ".$table_name." d on e.id=d.id and e.house_status=1 and e.status=99 $where group by d.relation order by cast(relation AS SIGNED)");
$result = $this->db->fetch_array($rs);
return $result;
}
public function communitylists_bycondition($data) {
$catid = intval($data['catid']);
...
$where = "and uid=$uid";
if(!$this->set_modelid($catid)) return false;
$rs = $this->db->query("select ".$condition.",count(".$condition.") as counts from ".$tablename." where house_status=1 and status=99 $where group by ".$condition." order by ".$condition);
$result = $this->db->fetch_array($rs);
return $result;
}
public function esflists_bycommunity($data) {
$catid = intval($data['catid']);
$order = $data['order'];
if(!$this->set_modelid($catid)) return false;
$tablename = $this->db->table_name;
...
if($catid=='112')
{
$urlrules = ESF_PATH.'jingjiren-esf-'.$data['userid'].'-g{$page}';
}
elseif($catid=='113')
{
$urlrules = ESF_PATH.'jingjiren-rent-'.$data['userid'].'-g{$page}';
}
define('URLRULE',$urlrules);
if($data['relation']) {
$relation = intval($data['relation']);
...
$sql.= " and `id` in ($ids)";
$this->db->table_name = $tablename;
$key_array = $this->db->listinfo($sql,$order,$page,$limit);
$pages = $this->db->pages;
}
unset($result);
}
...
{
$where = $sql.$data['where'];
$this->db->table_name = $tablename;
$key_array = $this->db->listinfo($where,$order,$page,$limit);
$pages = $this->db->pages;
}
$result['result'] = $key_array;
$result['pages'] = $pages;
return $result;
}
public function avgprice_sell($data)
{
$lastmonth = date("Ym",mktime(0,0 ,0,date("m")-1,1,date("Y")));
$this->avg_db = h5_base::load_model('community_avgprice_model');
$avginfo = $this->avg_db->select(array('cid'=>$data['id'],'type'=>'1','markdate'=>$lastmonth),"*",1,'addtime desc');
if($avginfo)
{
return $avginfo[0];
}
}
public function avgprice_history($data)
{
$avginfo = array();
$this->avg_db = h5_base::load_model('community_avgprice_model');
$avginfo[1] = $this->avg_db->select(array('cid'=>$data['id'],'type'=>1),"*",$data['limit'],'cast(markdate AS SIGNED) asc');
$avginfo[2] = $this->avg_db->select(array('cid'=>$data['id'],'type'=>2),"*",$data['limit'],'cast(markdate AS SIGNED) asc');
return $avginfo;
}
public function esfcounts($data) {
$catid = intval($data['catid']);
...
if($flag)
{
$sql.= " and (flag like '%1%' or flag like '%2%' or flag like '%3%' or flag like '%4%')";
}
$sql.= " and `uid` = $userid";
...
$sql.= " and `id` in ($ids)";
$this->db->table_name = $tablename;
$key_array = $this->db->select($sql,'id,title,unit,floor,hushu,xszt,shapan',$limit,$order,'','id');
}
}
return $key_array;
}
