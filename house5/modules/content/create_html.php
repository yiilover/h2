...
$this->url = h5_base::load_app_class('url');
...
$forward = $start ?"?s=content/create_html/update_urls/type/$type/dosubmit/1/first/$first/fromid/$fromid/toid/$toid/fromdate/$fromdate/todate/$todate/pagesize/$pagesize/page/$page/pages/$pages/total/$total/modelid/$modelid": preg_replace("/\/page\/([0-9]+)\/pages\/([0-9]+)\/total\/([0-9]+)/","/page/$page/pages/$pages/total/$total",$http_url);
}else {
delcache('url_show_'.$_SESSION['userid'],'content');
$message = L('create_update_success');
$forward = '?s=content/create_html/update_urls';
}
...
$forward = "?s=content/create_html/update_urls/set_catid/1/pagesize/$pagesize/dosubmit/1";
showmessage($message,$forward,200);
}
$catid_arr = getcache('update_url_catid'.'-'.$this->siteid.'-'.$_SESSION['userid'],'content');
$autoid = $autoid ?intval($autoid) : 0;
if(!isset($catid_arr[$autoid])) showmessage(L('create_update_success'),'?s=content/create_html/update_urls',200);
...
$forward = $start ?"?s=content/create_html/update_urls/type/$type/dosubmit/1/first/$first/fromid/$fromid/toid/$toid/fromdate/$fromdate/todate/$todate/pagesize/$pagesize/page/$page/pages/$pages/total/$total/autoid/$autoid/set_catid/1": preg_replace("/\/page\/([0-9]+)\/pages\/([0-9]+)\/total\/([0-9]+)/","/page/$page/pages/$pages/total/$total",$http_url);
}else {
$autoid++;
$message = L('updating').$this->categorys[$catid]['catname']." ...";
$forward = "?s=content/create_html/update_urls/set_catid/1/pagesize/$pagesize/dosubmit/1/autoid/$autoid";
}
showmessage($message,$forward,200);
}
}else {
...
$message = L('create_update_success');
$forward = '?s=content/create_html/show';
...
$percent = round($creatednum/$total,2)*100;
$message = L('need_update_items',array('total'=>$total,'creatednum'=>$creatednum,'percent'=>$percent));
$forward = $start ?"?s=content/create_html/show/type/$type/dosubmit/1/first/$first/fromid/$fromid/toid/$toid/fromdate/$fromdate/todate/$todate/pagesize/$pagesize/page/$page/pages/$pages/total/$total/modelid/$modelid": preg_replace("/\/page\/([0-9]+)\/pages\/([0-9]+)\/total\/([0-9]+)/","/page/$page/pages/$pages/total/$total",$http_url);
}else {
...
$forward = "?s=content/create_html/show/set_catid/1/pagesize/$pagesize/dosubmit/1";
showmessage($message,$forward,200);
}
if(count($catids)==1 &&$catids[0]==0) {
$message = L('create_update_success');
...
$autoid = $autoid ?intval($autoid) : 0;
if(!isset($catid_arr[$autoid])) showmessage(L('create_update_success'),'?s=content/create_html/show',200);
$catid = $catid_arr[$autoid];
$modelid = $this->categorys[$catid]['modelid'];
$this->db->set_model($modelid);
$table_name = $this->db->table_name;
...
$percent = round($creatednum/$total,2)*100;
$message = '¡¾'.$this->categorys[$catid]['catname'].'¡¿ '.L('have_update_items',array('total'=>$total,'creatednum'=>$creatednum,'percent'=>$percent));
$forward = $start ?"?s=content/create_html/show/type/$type/dosubmit/1/first/$first/fromid/$fromid/toid/$toid/fromdate/$fromdate/todate/$todate/pagesize/$pagesize/page/$page/pages/$pages/total/$total/autoid/$autoid/set_catid/1": preg_replace("/\/page\/([0-9]+)\/pages\/([0-9]+)\/total\/([0-9]+)/","/page/$page/pages/$pages/total/$total",$http_url);
}else {
$autoid++;
$message = L('start_update').$this->categorys[$catid]['catname']." ...";
$forward = "?s=content/create_html/show/set_catid/1/pagesize/$pagesize/dosubmit/1/autoid/$autoid";
}
showmessage($message,$forward,200);
}
}else {
...
$forward = "?s=content/create_html/category/set_catid/1/pagesize/$pagesize/dosubmit/1/modelid/$modelid/referer/$referer";
showmessage($message,$forward);
}
$catid_arr = getcache('update_html_catid'.'-'.$this->siteid.'-'.$_SESSION['userid'],'content');
$autoid = $autoid ?intval($autoid) : 0;
...
showmessage(L('create_update_success'),'?s=content/content/init/catid/'.$catid_arr[0],200);
}else {
showmessage(L('create_update_success'),'?s=content/create_html/category',200);
}
}
$catid = $catid_arr[$autoid];
$page = $page ?$page : 1;
...
$forward = "?s=content/create_html/category/set_catid/1/pagesize/$pagesize/dosubmit/1/autoid/$autoid/page/$page/total_number/$total_number/modelid/$modelid/referer/$referer";
}else {
$autoid++;
$message = $this->categorys[$catid]['catname'].L('create_update_success');
$forward = "?s=content/create_html/category/set_catid/1/pagesize/$pagesize/dosubmit/1/autoid/$autoid/modelid/$modelid/referer/$referer";
}
showmessage($message,$forward,200);
}else {
$show_header = $show_dialog  = '';
...
if(!in_array($catid,array('6','7','10')))
{
$add = "catid='$catid' AND ";
}
$rs = $this->db->select("$add id IN ($ids)");
$tablename = $this->db->table_name.'_data';
foreach($rs as $r) {
if($r['islink']) continue;
$this->db->table_name = $tablename;
