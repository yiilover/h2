...
$datas = $this->db->listinfo(array('siteid'=>$this->siteid,'type'=>0),'modelid',$_GET['page'],30);
$items = array();
...
$cache_api = h5_base::load_app_class('cache_api','admin');
...
if($is_exists) showmessage(L('operation_failure'),'?s=content/sitemodel/init');
...
$this->public_cache();
showmessage(L('operation_success'),'?s=content/sitemodel/init');
}
}else {
h5_base::load_sys_class('form','',0);
$show_validator = '';
...
$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?s=content/sitemodel/add\', title:\''.L('add_model').'\', width:\'580\', height:\'400\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);',L('add_model'));
...
$this->field_db = h5_base::load_model('sitemodel_field_model');
$field_array = array();
