...
<!--
	function select_list(obj,title,id) {
		var relation_ids = window.top.$(\'#investor\').val();
		...
		if($(obj).attr(\'class\')==\'line_ff9966\' || $(obj).attr(\'class\')==null) {
			...
			if(relation_ids !=\'\' ) {
				...
				window.top.$(\'#investor\').val(newrelation_ids);
			}
		} else {
			$(obj).attr(\'class\',\'line_ff9966\');
			var str = "<li id=\'"+sid+"\'>¡¤<span>"+title+"</span><a href=\'javascript:;\' class=\'close\' onclick=\\"remove_company(\'"+sid+"\',"+id+",\'investor\')\\"></a></li>";
			window.top.$(\'#investor_text\').append(str);
			if(relation_ids ==\'\' ) {
				window.top.$(\'#investor\').val(id);
			} else {
				relation_ids = relation_ids+\'|\'+id;
				window.top.$(\'#investor\').val(relation_ids);
			}
		}
}
//-->
