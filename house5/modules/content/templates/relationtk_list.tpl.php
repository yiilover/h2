...
<input type="hidden" value="content/content/public_relationtk_list" name="s">
...
<!--
	function select_list(obj,title,id) {
		var relation_ids = window.top.$(\'#xgtk\').val();
		...
		if($(obj).attr(\'class\')==\'line_ff9966\' || $(obj).attr(\'class\')==null) {
			...
			if(relation_ids !=\'\' && relation_ids !=0) {
				...
				window.top.$(\'#xgtk\').val(newrelation_ids);
			}
		} else {
			$(obj).attr(\'class\',\'line_ff9966\');
			var str = "<li id=\'"+sid+"\'>¡¤<span>"+title+"</span><a href=\'javascript:;\' class=\'close\' onclick=\\"remove_company(\'"+sid+"\',"+id+",\'xgtk\')\\"></a></li>";
			window.top.$(\'#xgtk_text\').append(str);
			if(relation_ids ==\'\' || relation_ids ==0) {
				window.top.$(\'#xgtk\').val(id);
			} else {
				relation_ids = id;
				window.top.$(\'#xgtk\').val(relation_ids);
			}
