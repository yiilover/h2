...
<!--
	$(function(){
		...
		$("#tablename").formValidator({onshow:"';echo L("input").L('model_tablename');echo '",onfocus:"';echo L("input").L('model_tablename');echo '"}).regexValidator({regexp:"^([a-zA-Z0-9]|[_]){0,20}$",onerror:"';echo L("model_tablename_wrong");;echo '"}).inputValidator({min:1,onerror:"';echo L("input").L('model_name');echo '"}).ajaxValidator({type : "get",url : "",data :"s=content/sitemodel/public_check_tablename",datatype : "html",async:\'false\',success : function(data){	if( data == "1" ){return true;}else{return false;}},buttons: $("#dosubmit"),onerror : "';echo L('model_tablename').L('exists');echo '",onwait : "';echo L('connecting');echo '"});		
	})
//-->
</script>
<div class="pad-lr-10">
<form action="?s=content/sitemodel/import" method="post" id="myform" enctype="multipart/form-data">
<fieldset>
  ...
  <tr>
		<th>';echo L('import_model');;echo '</th>
		<td>
...
<fieldset>
 ...
 <tr>
  <th width="200">';echo L('available_styles');;echo '£º</th>
        <td>
		';echo form::select($style_list,'','name="default_style" id="default_style" onchange="load_file_list(this.value)"',L('please_select'));echo ' 
		</td>
</tr>
		<tr>
...
<div class="bk15"></div>
<div class="btn"><input type="submit" id="dosubmit" name="dosubmit" value="';echo L('submit');;echo '" class="button"/></div> 
</form>
</div>
