...
<input type="hidden" value="relationlp" name="type">
<input type="hidden" value="';echo $id;;echo '" name="id">
...
<!--
	function select_list(obj,title,id) {
		var relation_ids = window.top.$(\'#xglp\').val();
		...
		if($(obj).attr(\'class\')==\'line_ff9966\' || $(obj).attr(\'class\')==null) {
			...
			if(relation_ids !=\'\' ) {
				...
				window.top.$(\'#xglp\').val(newrelation_ids);
			}
		} else {
			$(obj).attr(\'class\',\'line_ff9966\');
			var str = "<li id=\'"+sid+"\'>¡¤<span>"+title+"</span><a href=\'javascript:;\' class=\'close\' onclick=\\"remove_company(\'"+sid+"\',"+id+",\'xglp\')\\"></a></li>";
			window.top.$(\'#relation_lp_text\').append(str);
			if(relation_ids ==\'\' ) {
				window.top.$(\'#xglp\').val(id);
			} else {
				relation_ids = relation_ids+\'|\'+id;
				window.top.$(\'#xglp\').val(relation_ids);
			}
		}
}
//-->
