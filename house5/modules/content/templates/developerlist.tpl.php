...
<input type="hidden" value="developer" name="type">
...
<!--
	function select_list(obj,title,id) {
		var relation_ids = window.top.$(\'#developer\').val();
		...
		if($(obj).attr(\'class\')==\'line_ff9966\' || $(obj).attr(\'class\')==null) {
			...
			if(relation_ids !=\'\' ) {
				...
				window.top.$(\'#developer\').val(newrelation_ids);
			}
		} else {
			$(obj).attr(\'class\',\'line_ff9966\');
			var str = "<li id=\'"+sid+"\'>¡¤<span>"+title+"</span><a href=\'javascript:;\' class=\'close\' onclick=\\"remove_company(\'"+sid+"\',"+id+",\'developer\')\\"></a></li>";
			window.top.$(\'#developer_text\').append(str);
			if(relation_ids ==\'\' ) {
				window.top.$(\'#developer\').val(id);
			} else {
				relation_ids = relation_ids+\'|\'+id;
				window.top.$(\'#developer\').val(relation_ids);
			}
		}
}
//-->
