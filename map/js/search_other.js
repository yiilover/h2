...
var SearchOther ={
	...
	gotosearchnear:function(page)
	{
		...
		{
			var url = filePath + 'searchkeyword.php';
	...
	searchNear:function(nearName, nearType, page)
	{
		...
		{
			var url = filePath + 'searchnearhouse.php';
			//havecenter = 1;
			var pars = {
				'random':Math.random(),
	  ...
	  if(status == 0)
	    ...
	    var hits = result.hits;
			...
			for(var i=0; i<len; i++)
			{
				...
				if("" == V.pic)
				{
					V.pic = "http://img.soufun.comimage/tigong.gif";
				}
				var dotphoto = "http://img2s.soufun.com/map/images/near";
	...
	startonclick:function(name,hitX,hitY)
	{
		...
		else
		{
			...
			markerOpt.imageUrl = 'http://img2s.soufun.com/map/images/qd.png';
	...
	endonclick:function(name,hitX,hitY)
	{
		...
		{
			var markerOpt = new MMarkerOptions();
			markerOpt.imageUrl = 'http://img2s.soufun.com/map/images/zd.png';
			//markerOpt.anchor = new MPoint(-22, -27);
			markerOpt.imageSize = new MSize(43,27);
			markerOpt.hasShadow = true;
			var labelOpt = new MLabelOptions();
