...
var SFMap = {
	...
	searchKeyword:function(page, flg, keyword)
	{
		...
		{
			var checkboxs = {sell_pre:2,sell_now:1,sell_out:4,sell_last:3};//,rent_sell:4};
			...
			var kOnComplete = function(origRequest){
				...
				me.keyPointMarker = {name:keyword, x:cityx, y:cityy};
				...
				me.map.setCenter(me.keyPointMarker.y,me.keyPointMarker.x,mapsize);
				me.keepkw = true;
				me.searchResult(0,1);
			...
			var kMethod = 'post';
			var kParams = {'keyword':keyword, 'output':'json', 'random':Math.random()};
			var homeParams = {'city':city, 'cname':searchcondition['cityname'], 'citycode':curCity, 'cid':zssearch.cid, 'dearlertype':2};
			var type_cn = '楼盘';
			var url = 'op=searchkey_hs';
			var btype = 'house';
			if ('装饰公司' == me.desName)
			{
				if('house' == flg) kParams = SFUtil.objMerge(homeParams,kParams);
				type_cn = '公司';
				url = 'searchkey_zs.php';
				btype = 'zhuangshi';
			}
			...
			{
				keyPointUrl = filePath + 'op=searchkeyword'; 这个功能暂时隐掉了
				me.kwHouse = false;
				me.keepkw = true;
				kParams.city = searchcondition['cityname'];
				kParams.btype = btype;
			...
			if (!keepKeyword)
			{
				...
				if (sale_arr.length > 0) {
					...
					if(''!= purposes && purposes.indexOf('不限') < 0)
					{
						...
						params.purposes = temppurp[1];//改成传数字比较好
					}
				...
				if(me.firstLoad)
				{
					params.purpose = searchcondition.purpose;
					params.price = searchcondition.price;
					params.districts = searchcondition.district;
					params.params = searchcondition.area;
					if(params.districts) params.round = searchcondition.round;
					if(params.districts || params.round) params.subway = searchcondition.subwayname;
				}
				*/
			//params.keyword = keyword;
			}
	...
	info2url:
	{
		...
		detailUrl:function(info)
		{
			var purpose = info.purpose;
			var serverUrl = 'http://';
			if ((purpose.indexOf('写字楼') > -1) && this.officeCity)
			{
				serverUrl+='/house/'+citynameencode+'_';
			}
			else if ((purpose.indexOf('商铺') > -1) && this.shopCity)
			{
				serverUrl+='/house/'+citynameencode+'_';
			}
			else {
