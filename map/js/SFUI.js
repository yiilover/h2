...
var SFMapUI = {
	...
	newPurpose:function(purpose) {
		...
		{
			str += '<span><a href="javascript:;" onclick="SFMapUI.newPurpose(\'不限,-1\');SFMap.autoView=true;SFMap.searchResult();return false;">不限</a></span>';
		}
		for(var i=0; i<Purposes.length; i++)
		{
			...
			{
				str += '><a href="javascript:;" onclick="SFMapUI.newPurpose(\''+itemThis.name+','+itemThis.index+'\');SFMap.autoView=true;SFMap.searchResult();return false;">'+itemThis.name+'</a>';
			}
			str += '</span>';
		}
		var str1 = '<span class="top_zt">楼盘状态:'
			+'<label><input name="sell_now" id="sell_now" type="checkbox" value="1" onclick="SFMap.searchResult(0,1);" checked="checked"/><span class="blue" title="在售">在售</span></label>'
			+'<label><input name="sell_pre" id="sell_pre" type="checkbox" value="1" onclick="SFMap.searchResult(0,1);" checked="checked" /><span class="red" title="即将上市">即将上市</span></label>'
			+'<label><input name="sell_last" id="sell_last" type="checkbox" value="1" onclick="SFMap.searchResult(0,1);" /><span class="were" title="尾盘">尾盘</span></label>'
			+'<label><input name="sell_out" id="sell_out" type="checkbox" value="1" onclick="SFMap.searchResult(0,1);" /><span class="grey" title="售完">售完</span></label>';
		if(currentPurpose.indexOf('不限') == 0 || currentPurpose.indexOf('写字楼') == 0 || currentPurpose.indexOf('商铺') == 0)
		{
			//str1 += '<label><input name="rent_now" id="rent_now" type="checkbox" value="1" onclick="SFMap.searchResult(0,1);" /><span class="rentnow" title="在租">&nbsp;</span></label>';
			//	+'<label style="display:none;"><input name="rent_sell" id="rent_sell" type="checkbox" value="1" onclick="SFMap.searchResult(0,1);" /><span class="rentsell" title="租售">&nbsp;</span></label>';
		}
		str1 += '</span>';
	...
	newPrice:function(price) {
		...
		{
			str += '<span><a href="javascript:;" onclick="SFMapUI.newPrice(\'\');SFMap.autoView=true;SFMap.searchResult();return false;">不限</a></span>';
		}
		if (objPrice)
		{
			...
			var priceTable = function (tbhead, arrayPriceText, arrayPriceValue)
			{
				...
				{
					if(arrayPriceValue[i].indexOf('以下')>-1)
					{
						priceNum = arrayPriceValue[i].substr(0,arrayPriceValue[i].indexOf('以下'));
						pricecondition = "[0,"+priceNum+"]";
						priceDis = priceFormat(priceNum)+'以下';
					}
					else if(arrayPriceValue[i].indexOf('以上')>-1)
					{
						priceNum = arrayPriceValue[i].substr(0,arrayPriceValue[i].indexOf('以上'));
						pricecondition = "["+priceNum+",]";
						priceDis = priceFormat(priceNum)+'以上';
					}
					else
					{
					...
					{
						str += '><a href="javascript:;" onclick="SFMapUI.newPrice(\''+thisPrice+'\');SFMap.autoView=true;SFMap.searchResult();return false;">'+priceDis+'</a></span>';
					}
				}
				str += tbhead;
				return str;
	...
	*/
	...
	suggest_selected:0,
	suggest_url: filePath+'op=suggest',
