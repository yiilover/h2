...
class content_form {
	...
	function editor($field, $value, $fieldinfo) {
		...
		if(!$value) $value = $defaultvalue;
		if($minlength || $pattern) $allow_empty = '';
		if($minlength) $this->formValidator .= '
                $("#'.$field.'").formValidator({'.$allow_empty.'onshow:"",onfocus:"'.$errortips.'"}).functionValidator({
	    fun:function(val,elem){
                        if(typeof(CKEDITOR)!=\'undefined\'){
				var oEditor = CKEDITOR.instances.'.$field.';
				var data = oEditor.getData();			
			}else{
				var data = editor_'.$field.'.getContent();			
			}
	...
	function catid($field, $value, $fieldinfo) {
		...
		$publish_str = '';
		if(defined('IN_ADMIN') && ROUTE_A=='add') $publish_str = " <a href='javascript:;' onclick=\"omnipotent('selectid','?s=content/content/add_othors/siteid/".$this->siteid."','".L('publish_to_othor_category')."',1);return false;\" style='color:#B5BFBB'>[".L('publish_to_othor_category')."]</a><ul class='list-dot-othors' id='add_othors_text'></ul>";
	...
	window.top.art.dialog({title:'".L('cut_the_picture','','content')."', id:'crop', iframe:'index.php?s=content/content/public_crop/module/content/catid/'+catid+'/input/$field/preview/".($show_type && defined('IN_ADMIN') ? $field."_preview" : '')."&picurl='+encodeURIComponent(id), width:'680px', height:'480px'}, 	function(){var d = window.top.art.dialog({id:'crop'}).data.iframe;
	...
	function pages($field, $value, $fieldinfo) {
		...
		if($value) {
			...
			return "<select name=\"info[paginationtype]\" id=\"paginationtype\" onchange=\"if(this.value==1)\$('#paginationtype1').css('display','');else \$('#paginationtype1').css('display','none');\">
                ...
                <option value=\"2\" selected>".L('page_type3')."</option>
            </select>
	...
	function map($field, $value, $fieldinfo) {
		...
		return '<input type="button" name="'.$field.'_mark" id="'.$field.'_mark" value="'.$tips.'" class="button" onclick="omnipotent(\'selectid\',\'/api.php?op=map&field='.$field.'&modelid='.$modelid.'\',\''.L('mapmark','','map').'\',1,700,420)"><input type="hidden" name="info['.$field.']" value="'.$value.'" id="'.$field.'" >';
	}
