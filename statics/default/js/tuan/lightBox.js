...
function Box(objID,W,H,inner,footerInner,addr){
	...
	//创建html结构并赋id以及class		
		...
		if(inner==undefined || inner=="" ){inner="欢迎访问本站！"}
		if(addr==undefined || addr=="" ){inner=""}
		inner = inner.replace("add_hidden", addr);
		if(footerInner==undefined || footerInner=="" ){footerInner="www.house5.net &copy; 2012"}//页脚没有内容时默认值
		...
		objHead.innerHTML='<div class="headerL" id='+headerLid+'><span style="float:right;background:url(/statics/default/img/tuan/lightbox-close.png) no-repeat 0 center;display:inline-block;height:46px;width:16px;cursor:pointer;" id='+closeBtn+'></span></div><div class="headerR"></div>';
