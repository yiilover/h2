define(function(require, exports, module) {
	...
	return {
		...
		getNewFloors: function(e, b, c, a, d, m, n, f) {
			...
			var ajaxurl = "";
			if (searchType == 'xinfang') ajaxurl = SITE_URL + "api.php?op=gethouse&operate=getNewHouses&areaid=" + e + "&keywords=" + (params.keyword != '' ? encodeURIComponent(params.keyword) : '') + "&circleid=" + b + "&projecttype=" + c + "&buildfeature=" + a + "&price=" + d + "&minX=" + min_x + "&maxX=" + max_x + "&minY=" + min_y + "&maxY=" + max_y + '&random=' + Math.random()
			else {
		...
		getKeyHouse: function(k) {
			if (searchType == "xinfang") {
				$.ajax({
					url: SITE_URL + "api.php?op=getkeyhouse&operate=getNewHouses&keyword=" + encodeURIComponent(k),
					dataType: 'json',
					success: function(h) {
						if (h) {
		...
		openTipFrame: function(c, b) {
			...
			if (searchType == "xinfang") {
				$.ajax({
					type: "GET",
					url: SITE_URL + "api.php?op=housetip&o=getNewHouse&floorId=" + c,
					dataType: 'json'
				}).done(function(data) {
					if (data != null) {
						var url = "";
						if (data.aliasname != "noAlias") url = "<a href='" + HOUSE_URL + data.aliasname + ".html' target='_blank'>";
						else url = "<a href='" + HOUSE_URL + data.floorId + "/' target='_blank'>";
						var k = "&nbsp;";
						if (data.sellSchedule != "0") {
							k += "<img src='" + IMG_URL + "sellSchedule" + data.sellSchedule + ".gif' style='vertical-align:middle;'/>";
						}
						var bbs='';
						if(data.boardId)
							bbs='<a href="'+data.boardId+'" class="fr">进入业主论坛</a>';
						var company='';
						if(data.company!="暂无")
							company='<a target="_blank" href="' + HOUSE_URL + 'company-' + data.companyid + '.html">' + data.company + '</a>';
						else
						...
						alertM(html+list+page,{
							...
							of:function(){
								...
								var communityData=function(){
									$("#property_list table").html('<tr><td><img src="http://house.inhe.net/static2/images/map/8-0.gif" style="width:220px;height:19px;"></td></tr>');
									$.ajax({
										type: "GET",
										cache:false,
