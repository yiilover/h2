...
function ShowLilv(month, lt){
	...
	$("sdlv").innerHTML = myround(lilv_array[lt][1][year] * 100, 2) + "%";
	$("gjlv").innerHTML = myround(lilv_array[lt][2][year] * 100, 2);
}
...
function ext_total(fmobj){
	...
	if(fmobj.type.value == 3 ){
		...
		if(fmobj.jisuan_radio[0].checked == true){
			//------------ 根据单价面积计算
			if(!reg_Num(fmobj.price.value)){alert("请填写总价");fmobj.price.focus();return;}

			//房款总额
			var fangkuan_total = fmobj.price.value*10000;
			fmobj.fangkuan_total1.value = fangkuan_total;
			fmobj.fangkuan_total2.value = fangkuan_total;
			//贷款总额
			var daikuan_total = (fmobj.price.value*10000 ) * (fmobj.anjie.value / 10);
			fmobj.daikuan_total1.value = daikuan_total;
			fmobj.daikuan_total2.value = daikuan_total;
			//首期付款
...
function chanage_type_mm(){
	if($("dengeben2").checked)
		$("type1_mm1").innerHTML = '<span class="td_1">月均金额：</span><textarea name="month_money1" rows="5" class="guestbook02textarea" ></textarea>';
	else
		$("type1_mm1").innerHTML = '<span class="td_1">月均还款：</span><input name="month_money1" type="text" class="input" />元';
}
