...
function change_images(uploadid,returnid){
	...
	$.each( contents, function(i, n) {
		...
		str += "<li id='image"+ids+"'><input type='text' name='"+returnid+"_url[]' value='"+n+"' style='width:310px;' ondblclick='image_priview(this.value);' class='input-text'> <input type='text' name='"+returnid+"_alt[]' value='"+filename+"' style='width:160px;' class='input-text' onfocus=\"if(this.value == this.defaultValue) this.value = ''\" onblur=\"if(this.value.replace(' ','') == '') this.value = this.defaultValue;\"> <a href=\"javascript:remove_div('image"+ids+"')\">移除</a> <span value=\"设为标题图\" onclick=\"title_pic('"+n+"')\" name=\"title\">设为标题图</span></li>";
		});
...
//************显示关联楼盘/用于切换 add by L 2012.3.15************
function show_relation1(modelid,id) {
$.getJSON("?s=content/content/public_getjson_lp_ids&modelid="+modelid+"&id="+id, function(json){
	var newrelation_ids = '';
	if(json==null) {
		alert('没有添加相关楼盘');
		return false;
	...
	$.each(json, function(i, n){
		newrelation_ids += "<li id='"+n.sid+"'>·<span>"+n.title+"</span><a href='javascript:;' class='close' onclick=\"remove_company('"+n.sid+"',"+n.id+",'xglp')\"></a></li>";
	});

	$('#relation_lp_text').html(newrelation_ids);
}); 
}
//************显示相关文章楼盘/多表关联 add by L 2012.3.15************
function show_relation2(sourcemodelid,modelid,id) {
$.getJSON("?s=content/content/public_getjson_ids&modelid="+modelid+"&sourcemodelid="+sourcemodelid+"&id="+id, function(json){
	var newrelation_ids = '';
	if(json==null) {
		alert('没有添加相关文章');
		return false;
...
//显示公司/多表关联 add by L 2012.3.15 ************
function show_company(sourcemodelid,modelid,id,type) {			// sourcemodelid 为要添加项的模型id add by L 2012.3.16
$.getJSON("?s=content/content/public_getjson_company_ids&modelid="+modelid+"&sourcemodelid="+sourcemodelid+"&type="+type+"&id="+id, function(json){
	var newrelation_ids = '';
	if(json==null) {
		alert('没有添加相关信息');
		return false;
	...
	$.each(json, function(i, n){
		newrelation_ids += "<li id='"+n.sid+"'>·<span>"+n.title+"</span><a href='javascript:;' class='close' onclick=\"remove_company('"+n.sid+"',"+n.id+",'"+type+"')\"></a></li>";
	});

	$('#'+type+'_text').html(newrelation_ids);
}); 
}
...
//************移除相关楼盘/公司 add by L 2012.3.15************
function remove_company(sid,id,type) {
	var relation_ids = $('#'+type).val();
	if(relation_ids !='' ) {
		$('#'+sid).remove();
		var r_arr = relation_ids.split('|');
		var newrelation_ids = '';
		...
		$('#'+type).val(newrelation_ids);
	}
}
//显示相关文章
function show_relation(modelid,id) {
$.getJSON("?s=content/content/public_getjson_ids&modelid="+modelid+"&id="+id, function(json){
	var newrelation_ids = '';
	if(json==null) {
		alert('没有添加相关文章');
		return false;
