...
UE.plugins['pagebreak'] = function () {
    ...
    me.addInputRule(function(root){
        root.traversal(function(node){
			var re=/\[page\](.+)\[\/page\]/;
            if(node.type == 'text' && me.options.pageBreakTag){
				subtitle = node.data.match(re);
				if (subtitle) {
					delete node.data;
					var hr = UE.uNode.createElement('<hr subtitle="'+subtitle[1]+'" class="subtitle" noshade="noshade" size="8" style="-webkit-user-select: none;">');
					node.children = [];
    ...
    me.addOutputRule(function(node){
        utils.each(node.getNodesByTagName('hr'),function(n){
			...
			else if(n.getAttr('class') == 'subtitle'){
				delete node.tag;
                var txt = UE.uNode.createText('[page]'+n.attrs['subtitle']+'[/page]');
                n.parentNode.insertBefore(txt,n);
                n.parentNode.removeChild(n);
            }
        })
...
(function () {
    ...
    var dialogBtns = {
        ...
        ok:['attachment', 'anchor', 'link', 'insertimage', 'map', 'gmap', 'insertframe', 'wordimage',
            'insertvideo', 'insertframe', 'edittip', 'edittable', 'edittd', 'scrawl', 'template', 'music', 'background','subtitle']
