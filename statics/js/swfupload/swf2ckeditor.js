function flashupload(uploadid, name, textareaid, funcName, args, module, catid, authkey) {
	...
	window.top.art.dialog({title:name,id:uploadid,iframe:'/index.php?s=attachment/attachments/swfupload'+args+setting,width:'500',height:'420'}, function(){ if(funcName) {funcName.apply(this,[uploadid,textareaid]);}else {submit_ckeditor(uploadid,textareaid);}}, function(){window.top.art.dialog({id:uploadid}).close()});
}
...
function submit_videos(uploadid,returnid){
	var d = window.top.art.dialog({id:uploadid}).data.iframe;
	var in_content = d.$("#att-status").html().substring(1);
	var in_content = in_content.split('|');
	IsFlv(in_content[0]) ? $('#'+returnid).attr("value",in_content[0]) : alert('选择的类型必须为flv/mp4');
}
...
function insert2editor(id,in_content,del_content) {	
	...
	$.get("/index.php?s=attachment/attachments/swfdelete",{data: del_content},function(data){});
